...
커밋 두 건의 실제 코드를 확인한 결과를 반영해 릴리즈 노트를 작성했습니다.
...
커밋 내역을 사용자 친화적인 릴리즈 노트로 정리했습니다.
...
${l}
${o}`,url:o}),"share"}catch(h){if(h instanceof Error&&(h.name==="AbortError"||h.message.includes("abort")||h.message.includes("canceled")||h.message.includes("cancel")))return"share"}}if(s&&typeof navigator<"u"&&navigator.share)try{return await navigator.share({title:c,text:`${e} 크루에 합류하세요!
${l}`,url:o}),"share"}catch(f){if(f instanceof Error&&(f.name==="AbortError"||f.message.includes("abort")||f.message.includes("canceled")||f.message.includes("cancel")))return"share"}if(await Fe())try{const f=r??(window.location.protocol==="https:"?`${a}/runc-icon.png`:void 0);return window.Kakao.Share.sendDefault({objectType:"feed",installTalk:!1,content:{title:`[RUNC] ${e} 크루에 초대합니다!`,description:l,...f&&{imageUrl:f},link:{mobileWebUrl:o,webUrl:o}},buttons:[{title:"RUNC 열기",link:{mobileWebUrl:o,webUrl:o}}]}),"kakao"}catch{}if(typeof navigator<"u"&&navigator.share)try{return await navigator.share({title:c,text:`${e} 크루에 합류하세요!
${l}`,url:o}),"share"}catch(f){if(f instanceof Error&&(f.name==="AbortError"||f.message.includes("abort")||f.message.includes("canceled")||f.message.includes("cancel")))return"share"}return await navigator.clipboard.writeText(`[RUNC] ${e} 크루에 합류하세요!
${l}
${o}`),"clipboard"}async function Td({runnerName:e,crewName:n,crewId:t,weekNumber:r,runNumber:i,distance:a,duration:o,pace:l,imageUrl:c,imageBlob:s}){const u=window.location.origin;let f=n;r&&i?f=`${n} ${r}주차 ${i}Run 완료`:r?f=`${n} ${r}주차`:i&&(f=`${n} ${i}Run 완료`);const h=new URLSearchParams;t&&h.set("crewId",t),h.set("crewName",n),h.set("runnerName",e),h.set("label",f),h.set("distance",String(a)),h.set("duration",o),l&&h.set("pace",l),c&&h.set("imageUrl",c);const p=`${u}/#${se(Y.shareRun,{})}?${h.toString()}`,x=l?` | ⚡ 페이스: ${l}/km`:"",y=`🏃‍♂️ ${e}님의 ${a}km 러닝 인증!`,_=`⏱️ 시간: ${o}${x}
...
${f}
${c}`),"clipboard"}async function Dd({memberName:e,crewName:n,crewId:t}){const r=window.location.origin,i=t?`${r}/#${se(Y.crewSquad,{crewId:t})}`:r,a=`[${n}] ${e}님 완납 확인!`,o=`${e}님, 벌금/회비 전액 완납 확인했습니다. 협조해주셔서 감사합니다 🙏🎉`;if(pn()&&typeof navigator<"u"&&navigator.share)try{return await navigator.share({title:a,text:`${o}
